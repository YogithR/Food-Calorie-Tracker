...
<div class="app-header">
  <div class="header-row">
    <div class="brand">
      ...
      <div>
        <h1 class="brand-title">MealSnap AI</h1>
        <p class="brand-sub">AI-powered calorie and nutrition tracker</p>
      </div>
    </div>
...
@st.cache_resource
def load_food_gate_model():
    # Lightweight ImageNet model used only for food-vs-non-food gating.
...
def is_food_image_with_imagenet(img: Image.Image, gate_model) -> tuple:
    """
    Returns:
    ...
    try:
        ...
        preds = gate_model.predict(x, verbose=0)
        ...
        # Fail open: if this check fails, rely on Food-101 confidence rules.
        return True, f"Food gate unavailable: {e}", 1.0
...
model = load_model()
food_gate_model = load_food_gate_model()
class_names = load_class_names()
nutrition_df = load_nutrition()

# ============================================================
# 11) DASHBOARD LAYOUT (left controls, right outputs)
# ============================================================
st.sidebar.caption("Secondary panel")
st.sidebar.info("Main workflow is available in the dashboard.")
